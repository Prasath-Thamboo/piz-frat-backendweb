...
//
// Séparé de mobile-auth.ts (getMobileUser/requireMobileRole) qui importe
// next/server : ce fichier-ci reste importable tel quel par server.ts, exécuté
// via `node` en dehors du bundler Next, où `next/server` (sans extension) ne
// se résout pas sous la résolution ESM stricte de Node.
